<?php
  ...
  **/
  ...
  class Validate
  {
    ...
    private static $error_regex = 'Some characters are not permitted';
    private static $error_fileSize = 'The file size is too big';
    private static $error_fileType = 'This file format is not permitted';
    private static $error_fileNotExist = 'here is a file with that name, if your file is different please rename it and try again';
    private static $error_fileRequired = 'You must upload a file';
    private static $error_exists = 'This does not exist';
    private static $error_notExists = 'This is used already';
    private static $error_rtf = 'There are non-permitted characters; you may only use %s tags';
    ...
    function __construct($post, $files = null)
    {
      ...
      foreach ($post as $key => $value) {
        $this->validObjects[$key] = new validateObject($value);
      }
    }
    ...
    //used for the rich text box with TinyMCE editor. Need to specify an array of acceptable tags array('p','ul','li','b','i','u','a','span')
    function rtf($tags, $errorMsg = null)
    {
      if ($this->isValid && (!empty($this->currentObject->value))) {
        $this->isValid = ($this->currentObject->value == strip_tags($this->currentObject->value, $tags)) ? true : false;

        if (!$this->isValid) {
          $this->setErrorMsg($errorMsg, self::$error_rtf, implode(", ", $tags));
        }
      }
      return $this;
    }
    ...
    //used to check if there is an entry in the database
    function exists($model, $method, $errorMsg = null)
    {
      if ($this->isValid && (!empty($this->currentObject->value))) {
        //set the db model
        $this->dbModel = new $model;
        //run the query
        $this->isValid = ($this->dbModel->$method($this->currentObject->value));
        if (!$this->isValid) {
          $this->setErrorMsg($errorMsg, self::$error_exists);
        }
      }
      return $this;
    }
    ...
    //used to check if there is an entry in the database
    function notExist($model, $method, $errorMsg = null)
    {
      if ($this->isValid && (!empty($this->currentObject->value))) {
        //set the db model
        $this->dbModel = new $model;
        //run the query
        $this->isValid = ($this->dbModel->$method($this->currentObject->value)) ? false : true;
        if (!$this->isValid) {
          $this->setErrorMsg($errorMsg, self::$error_notExists);
        }
      }
      return $this;
    }
    ...
    //used to check the the file type is allowed - ->fileType(array('image/jpeg', 'image/gif'))
    function fileNotExist($model, $method, $errorMsg = null)
    {
      if ($this->isValid && (!empty($this->currentFileObject->fileName))) {
        $finfo = new finfo();
        if (is_array($this->currentFileObject->tmpName)) {
          ...
          foreach ($this->currentFileObject->tmpName as $fileName) {
            if (!empty($fileName)) {
              ...
              $this->dbModel = new $model;
              //run the query
              $this->isValid = ($this->dbModel->$method($this->currentFileObject->fileName)) ? false : true;
            }
            if (!$this->isValid) {
              $this->setFileErrorMsg($errorMsg, self::$error_fileNotExist);
            }
            $i++;
          }
        } else {
    ...
    //used to check the the file type is allowed - ->fileType(array('image/jpeg', 'image/gif'))
    function fileRequired($errorMsg = null)
    {
      if ($this->isValid) {
        $this->isValid = (!empty($this->currentFileObject->fileName)) ? true : false;
        if (!$this->isValid) {
          $this->setFileErrorMsg($errorMsg, self::$error_fileRequired);
        }
      }
      return $this;
    }
